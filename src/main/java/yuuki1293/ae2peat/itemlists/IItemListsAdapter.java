...
public interface IItemListsAdapter {
    ...
    /**
     * Returns the first element from the list of {@link PatternContainerGroup} that corresponds to the list of
     * machines.
     */
    default Optional<PatternContainerGroup> findFirst(List<PatternContainerGroup> groups,
        List<? extends ItemLike> machines) {
        ...
        for (var group : groups) {
            var searchName = group.name()
                .getString()
                .toLowerCase();

            var matched = localizedNames.stream()
                .filter(m -> m.equals(searchName))
                .findFirst();
